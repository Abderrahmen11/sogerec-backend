    ...
    public function index()
    {
        try {
            ...
            $recentTickets = Ticket::with('user')->latest()->limit(3)->get()->map(function ($t) {
                $userName = $t->user ? $t->user->name : 'Unknown User';
                return "New client request submitted: {$t->title} by {$userName}";
            });

            $recentInterventions = Intervention::with('user')->where('status', 'completed')->latest()->limit(2)->get()->map(function ($i) {
                $userName = $i->user ? $i->user->name : 'Unknown Tech';
                return "Technician {$userName} completed intervention #{$i->id}";
            });
            ...
            \Log::error('AdminStats Error: ' . $e->getMessage(), [
                'exception' => $e,
